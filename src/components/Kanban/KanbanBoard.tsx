...
export const KanbanBoard: React.FC = () => {
  ...
  const getColumnHeaderBadge = (colId: string) => {
    switch (colId) {
      case 'backlog':
        return {
          label: 'BACKLOG',
          bg: 'bg-black text-zinc-300 border border-zinc-800/80',
  ...
  return (
    <div className="h-[calc(100vh-8.5rem)] flex flex-col gap-6 relative animate-fade-in-up">
      ...
      {/* 1. Filter Bar */}
      <div className="flex flex-col md:flex-row gap-4 items-center justify-between bg-[#080809] p-3 rounded-xl border border-zinc-800">

        {/* Search & Basic Filters */}
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 w-full md:w-auto">
          <div className="relative w-full sm:w-64 group">
            <FiSearch className="w-4 h-4 text-zinc-400 absolute left-3 top-1/2 -translate-y-1/2 group-focus-within:text-white transition-colors" />
            <input
              type="text"
          ...
          <button
            ...
            className={`flex items-center justify-center gap-1.5 px-3 py-2.5 text-xs font-semibold rounded-lg border transition-colors shadow-sm ${
              showFilters 
                ...
                : 'bg-black border-zinc-800 text-zinc-400 hover:text-zinc-200 hover:bg-zinc-900 hover:border-zinc-700'
            }`}
          >
            <FiFilter className="w-3.5 h-3.5" />
            <span>Filters</span>
            {(priorityFilter !== 'all' || statusFilter !== 'all' || dueDateFilter !== 'all' || tagFilter !== 'all') && (
              <span className="w-1.5 h-1.5 rounded-full bg-[#ededed] ml-1 shadow-[0_0_8px_rgba(59,130,246,0.5)]"></span>
            )}
          </button>
        </div>
      ...
      {showFilters && (
        <div className="bg-[#080809] p-4 rounded-xl border border-zinc-800 flex flex-wrap gap-4 panel-in z-10 shadow-lg">
          <div className="flex flex-col gap-1.5 w-full sm:w-auto min-w-[140px]">
            ...
            <select
              ...
              className="w-full text-xs px-2.5 py-2 rounded-lg bg-black border border-zinc-800 text-white focus:outline-none focus:border-zinc-600 transition-colors cursor-pointer"
            >
          ...
          <div className="flex flex-col gap-1.5 w-full sm:w-auto min-w-[140px]">
            <label className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest">Tag</label>
            <select
              value={tagFilter}
          ...
          {(priorityFilter !== 'all' || statusFilter !== 'all' || dueDateFilter !== 'all' || tagFilter !== 'all') && (
            <div className="flex items-end pb-0.5 ml-auto">
              <button 
                ...
                className="text-xs font-medium text-zinc-400 hover:text-white px-3 py-2 bg-black hover:bg-zinc-900 border border-zinc-800 rounded-lg transition-colors cursor-pointer"
              >
                Clear Filters
              </button>
            </div>
          ...
          return (
            <div
              ...
              className={`w-[320px] shrink-0 snap-start flex flex-col h-full rounded-2xl border transition-all duration-200 ${isOver
                  ? 'bg-black border-zinc-600 shadow-2xl scale-[1.01]'
                  : 'bg-[#080809] border-zinc-800'
                }`}
            >
              {/* Column Header */}
              <div className="p-4 flex items-center justify-between border-b border-zinc-800">
                <div className="flex items-center gap-2.5">
                  ...
                  <span className="text-xs font-bold text-zinc-400 bg-black px-2 py-0.5 rounded border border-zinc-800">
                    {colTasks.length}
                  </span>
                </div>
                <div className="flex items-center gap-1">
                  ...
                  <button className="p-1.5 text-zinc-600 hover:text-zinc-400 rounded-lg hover:bg-black cursor-pointer transition-colors">
              ...
              <div className="flex-1 overflow-y-auto p-3.5 space-y-3.5 custom-scrollbar">
                {colTasks.length === 0 ? (
                  <div className="h-32 border-2 border-dashed border-zinc-800/80 rounded-xl flex flex-col items-center justify-center text-center p-4 text-zinc-400 bg-zinc-900/20">
            ...
            >
              <div className="flex items-center justify-between pb-2 border-b border-zinc-800/50">
                <div className="flex items-center gap-2 text-white">
                  <div className="w-6 h-6 rounded-md bg-black border border-zinc-800 flex items-center justify-center">
                ...
                <button
                  onClick={() => setShowGitDrawer(false)}
                  className="text-zinc-400 hover:text-white text-xxs font-bold uppercase tracking-wider border border-zinc-800 hover:bg-zinc-900 px-2.5 py-1 rounded-lg cursor-pointer transition-colors"
                >
                  Close
                </button>
              </div>
              ...
              <div className="flex-1 overflow-y-auto space-y-3 pr-1 custom-scrollbar">
                {githubIssues.map((issue) => (
                  <div
                    ...
                    className="p-3.5 rounded-xl border border-zinc-800 bg-[#080809] hover:bg-black hover:border-zinc-700 cursor-pointer text-left flex flex-col gap-2 group transition-all duration-200 shadow-sm"
                  >
                    ...
                    <div className="flex justify-end items-center gap-1.5 text-[9px] font-bold uppercase tracking-wider text-zinc-400 group-hover:text-white mt-1 transition-colors">
