  ...
  if (!githubToken) {
    return (
      <div className="max-w-md mx-auto my-16 glass-panel rounded-2xl p-8 text-center flex flex-col items-center gap-5">
        <div className="p-4 rounded-2xl bg-black border border-zinc-800 text-zinc-400">
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <div className="lg:col-span-2 space-y-6">
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
            <div className="flex items-center gap-3.5">
              ...
              <div>
                ...
                <p className="text-[10px] text-zinc-400">Connected · Live API sync</p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 text-[10px] font-semibold text-zinc-400 border border-zinc-800 rounded-lg hover:bg-zinc-900 transition-colors cursor-pointer disabled:opacity-50"
              ...
              <button
                ...
                className="flex items-center gap-1 px-3 py-1.5 border border-zinc-800 hover:border-zinc-700 hover:text-zinc-200 rounded-lg text-[10px] font-semibold text-zinc-400 cursor-pointer transition-colors"
              >
        ...
        <div className="glass-panel rounded-2xl p-5">
          ...
          {/* Tab header */}
          <div className="flex overflow-x-auto hide-scrollbar bg-black p-1 rounded-xl border border-zinc-800 gap-1 mb-5">
            {TAB_CONFIG.map(tab => {
            ...
            {activeTab === 'commits' && (
              <div className="space-y-2 max-h-[380px] overflow-y-auto">
                {githubCommits.length === 0 ? (
                  <p className="text-xs text-zinc-400 italic text-center py-8">No commits in the visible event window.</p>
                ) : (
                  githubCommits.map(c => (
                    <div
                      key={c.id}
                      className="p-3 rounded-xl border border-zinc-800 bg-black flex items-center justify-between gap-3"
                    >
                      ...
                      <span className="text-[9px] font-semibold text-zinc-400 shrink-0 bg-black px-2 py-0.5 rounded-md border border-zinc-800">
                        {c.date}
                      </span>
                    </div>
                  ))
